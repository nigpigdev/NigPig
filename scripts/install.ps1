...
foreach ($tool in $pdTools) {
    Write-Host "   Kuruluyor: $($tool.name)" -ForegroundColor White
    $result = & go install -v $tool.pkg 2>&1
    if ($LASTEXITCODE -eq 0) {
        Write-Host "   [+] $($tool.name) kuruldu" -ForegroundColor Green
    }
    else {
        Write-Host "   [!] $($tool.name) kurulamadi" -ForegroundColor Yellow
    }
}
...
$extraTools = @(
    ...
    @{name = "gowitness"; pkg = "github.com/sensepost/gowitness@latest" }
)
...
foreach ($tool in $extraTools) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "   [!] $($tool.name) kurulamadi (opsiyonel)" -ForegroundColor Yellow
    }
}
...
if (-not ($env:PATH -like "*$buildDir*")) {
    ...
    Write-Host "   `$env:PATH += `";$buildDir`"" -ForegroundColor White
}
